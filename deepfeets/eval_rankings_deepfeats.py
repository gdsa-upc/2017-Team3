...
def display(params,query_id,ranking,relnotrel):
    ...
    query_im =  cv2.imread(os.path.join(params['root'],params['database'],
                           ...
                           query_id))
                           """
    query_im = mpimg.imread(os.path.join(params['root'],params['database'],
                           params['split'], 'images',
                           query_id))

    # Blue contour for the query
    #query_im = cv2.cvtColor(query_im,cv2.COLOR_BGR2RGB)
    #query_im = cv2.cvtColor(query_im, cv2.COLOR_2RGB)
    ...
    for i in range(15):
        ...
        # Read image
        im = mpimg.imread(os.path.join(os.path.join(params['root'],params['database'],
...
def load_ranking(params,query_id, annotation_val):
    ...
    ranking = pd.read_csv(os.path.join(params['root'],params['root_save'],
                        params['rankings_dir'],'deepfeats',query_id.split('.')[0] + '.txt'),
                        header= None)
